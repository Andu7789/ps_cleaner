-- Booking-time questions the cleaner actually needs answered before a
-- visit: whether the customer wants to meet the cleaner first, parking,
-- pets, and how the cleaner gets in. Structured columns rather than
-- folding into the free-text `notes` field, since these are things a
-- cleaner should see reliably in the job detail, not have to parse out of
-- prose.
alter table "PS_CLEAN_bookings"
  add column wants_meet_cleaner_first boolean not null default false,
  add column parking_available boolean not null default false,
  add column has_pets boolean not null default false,
  add column access_method text not null default 'let_in' check (access_method in ('keys', 'let_in'));

-- ps_clean_create_booking must be DROPPED first, not just CREATE OR
-- REPLACEd — a different parameter list creates a new overload instead of
-- replacing the function, which is exactly the PGRST203 ambiguity bug
-- fixed once already in this schema (see DECISIONS.md #ps_clean_0019b /
-- the "function overload ambiguity" entry). New params are appended with
-- defaults so every existing caller (recurring-booking generation, the
-- cron job, any admin script) keeps working unchanged.
drop function if exists ps_clean_create_booking(uuid, uuid, uuid, uuid, timestamptz, text, uuid[]);
...
grant execute on function ps_clean_create_booking(uuid, uuid, uuid, uuid, timestamptz, text, uuid[], boolean, boolean, boolean, text) to authenticated;
