...
- [ ] **"Build your experience" pricing calculator** — some competitor sites let a customer enter number of bedrooms/bathrooms/etc. and compute the price live as they go, rather than picking from a fixed service list. Would need: either a per-service set of quantity-based price modifiers (e.g. "£X per extra bedroom") or a separate quote-builder flow entirely, plus deciding how that interacts with the existing fixed-price service catalog, add-ons, and deposit calculation. Not started — a real feature, not a small tweak, needs its own design pass before building.
