-- Extends ps_clean_create_booking to accept optional add-ons. Deliberately
-- kept simple: an add-on's price is folded straight into the booking's
-- price_pence (the grand total for the whole job), and the existing
-- deposit/balance split (DECISIONS.md #6) applies to that total unchanged
-- — deposit_pence stays whatever the service configures; add-ons just
-- change what the eventual balance covers. No separate
-- "add-ons pay in full at booking" carve-out, since that would mean two
-- different payment-timing rules to reason about instead of one.
...
begin
  ...
  -- Every requested add-on must actually be offered for this service and
  -- currently active — re-validated server-side rather than trusting
  -- whatever total the client sent, same principle as everything else
  -- price-related in this function.
...
grant execute on function ps_clean_create_booking(uuid, uuid, uuid, uuid, timestamptz, text, uuid[]) to authenticated;
