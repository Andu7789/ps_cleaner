...
- [x] **"Build your experience" pricing calculator** — a customer enters a count per room type (admin-managed, e.g. Bedroom/Bathroom/Living Room, each with its own price) and the price updates live. Off by default: admin adds room types and opts individual services in via a checkbox, nothing appears automatically. `/admin/services` (room type management + per-service toggle), `PricingCalculator` on the booking page when a service has it enabled. Now also adjusts job duration and slot availability, not just price — see DECISIONS.md #25 for why this turned out to be much smaller/safer than DECISIONS.md #19 originally feared.
...
- ~~Pricing calculator: adjust job duration/slot availability, not just price~~ — done, see DECISIONS.md #25.
