...
Not yet done: **connecting the three `/api/cron/*` routes to an actual schedule** — infrastructure for this is built (Cloudflare Workers Cron Triggers, see DECISIONS.md #13) but deployment itself is currently blocked by an upstream `@opennextjs/cloudflare`/Next.js 16.3.4 incompatibility. Needs a decision: wait for an upstream fix, pin older package versions, or switch to Vercel (blocked only on a one-time interactive login). This pass's own testing was curl-based rather than a real headless-browser click-through, since no browser automation binary was available in this environment initially (Playwright was later installed mid-session and used for the calendar redesign specifically).
