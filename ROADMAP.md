...
- [x] Pre-visit questions at checkout (meet the cleaner first? parking nearby? pets? key access or let-in?) — stored as real columns on the booking, shown to the cleaner on the job detail page under "Before you arrive". See DECISIONS.md #18.
- [x] **"Build your experience" pricing calculator** — a customer enters a count per room type (admin-managed, e.g. Bedroom/Bathroom/Living Room, each with its own price) and the price updates live. Off by default: admin adds room types and opts individual services in via a checkbox, nothing appears automatically. Price-only — doesn't adjust a job's duration/slot availability, deliberately (see DECISIONS.md #19 for why). `/admin/services` (room type management + per-service toggle), `PricingCalculator` on the booking page when a service has it enabled.
- [x] Admin business settings page (business name, contact info, reminder/balance-charge timing) — `/admin/settings`, owner-only write (matches the existing RLS policy on `PS_CLEAN_business_settings`). Previously only editable via direct SQL.
- [x] Cleaner job map + "Get directions" — same pattern as Root Cafe App's site picker (no API key, no SDK): a Google Maps query-embed iframe for a preview, plus a plain directions deep link that hands off to the cleaner's own maps app for live turn-by-turn navigation. See DECISIONS.md #23.
- [x] One-press invoice demo data (`/admin/demo-data`) — adds real completed jobs across active cleaners so the invoicing feature has something to generate from, without needing a raw SQL console (which was the literal ask, but too big a security surface on a Supabase project shared with other apps — see DECISIONS.md #24).
- [ ] **Pricing calculator: adjust job duration/slot availability, not just price.** Right now a room-type selection (e.g. more bedrooms) only changes what a job costs, never how long the cleaner is actually booked for — deliberately, since doing it properly means `ps_clean_available_slots()` also needs to know the room counts to compute correct free windows, a materially bigger and riskier change to the core double-booking-prevention engine (see DECISIONS.md #4) than was safe to do in one night. Worth revisiting with its own dedicated live-testing pass, the same rigor every other change to that function has gotten (DECISIONS.md #7, #11, #19).
...
- ~~Email now sends for real, but from Resend's sandbox address, since this Resend account's only verified domains belonged to the user's other apps~~ — done: PS Cleaning now has its own domain (`psclean.site`, DECISIONS.md #21), verified with Resend (DKIM/SPF/MX all green), and `NOTIFY_FROM_EMAIL` sends as `bookings@psclean.site` — real mail to any recipient, not just the Resend account owner's own inbox (the old sandbox-sender limit from DECISIONS.md #15).
- ~~Only one admin exists, inviting more has no UI~~ — done: `/admin/team` (owner-only) invites by email, same first-login-linking pattern as cleaners. See DECISIONS.md #22.
- [x] Editing a cleaner's own details (name/email/phone/bio) after creation — previously only qualifications/hours/time-off/pay-rate were editable. See DECISIONS.md #22.
